...
//import 'package:angular/core_dom/common.dart';
    ...
    var inj = zone.run(() {
      ...
      initializeDateFormatting(null, null).then((_) {
        try {
          ...
          ASTParser astParser = injector.get(ASTParser);
          FormatterMap frms = injector.get(FormatterMap);
          var ngBindDirective = injector.get(MetadataExtractor)(NgBind)[0];

          // Add ng-binding to the body element
          var bodyElt = element.querySelector('body');
          bodyElt.classes.add('ng-binding');
          element.classes.add('ng-binding');
          var elementBinders = [
            new TaggedElementBinder(null, -1, true),
            new TaggedElementBinder(new ElementBinder(
              injector.get(Profiler),
              injector.get(Expando),
              injector.get(Parser),
              injector.get(CompilerConfig),
              null, [
                new DirectiveRef(bodyElt, NgBind,
                                 ngBindDirective,
                                 new Key(NgBind), "greeting", null)
                  ..mappings.add(new MappingParts('ng-bind',
                                                   astParser('greeting', formatters: frms),
                                                   '=>',
                                                   astParser('value'),
                                                   ngBindDirective.map[0]))
                ], [], {}, Directive.COMPILE_CHILDREN), -1, false)
          ];

          ViewFactory viewFactory = new TaggingViewFactory(rootElements, elementBinders, injector.get(Profiler));
